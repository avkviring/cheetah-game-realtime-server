...
impl PrefabResolver {
	...
	pub fn new(prefab: Prefab, groups: &GroupResolver, path: &Path) -> Result<Self, Error> {
		...
		let defaults = prefab
			.fields
			.iter()
			.filter_map(|field| field.field.clone().into_value().map(|value| (field_names[&field.name], value)))
			.collect();
	...
	pub fn resolve(&self, base: Vec<ObjectField>, extend: Vec<ExtendField>, path: &Path) -> Result<relay::GameObjectFieldsTemplate, Error> {
