...
fn should_set_float_counter_from_ffi() {
	...
	assert!(matches!(&command,C2SCommandUnion::SetFloatCounter(ref float_counter) if float_counter.global_object_id == 100));
...
fn should_increment_float_counter_from_ffi() {
	...
	assert!(matches!(&command,C2SCommandUnion::IncrementFloatCounter(ref float_counter) if float_counter.global_object_id == 100));
