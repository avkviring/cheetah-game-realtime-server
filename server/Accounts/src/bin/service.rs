...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let redis_port = cheetah_libraries_microservice::get_env("REDIS_PORT").parse().unwrap();
	let redis_auth = std::env::var("REDIS_AUTH").ok();

	let pg_user = cheetah_libraries_microservice::get_env("POSTGRES_USER");
	let pg_passwd = cheetah_libraries_microservice::get_env("POSTGRES_PASSWORD");
	let pg_db = cheetah_libraries_microservice::get_env("POSTGRES_DB");
	let pg_host = cheetah_libraries_microservice::get_env("POSTGRES_HOST");
	let pg_port: u16 = cheetah_libraries_microservice::get_env("POSTGRES_PORT").parse().unwrap();
