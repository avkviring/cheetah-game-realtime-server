...
impl Storage {
    ...
    async fn do_attach(&self, user: user::Id, cookie: &str) -> Result<(), AttachError> {
        ...
        let result: Result<_, sqlx::Error> =
            sqlx::query("insert into cookie_users (user_id,cookie) values($1,$2)")
        ...
        result.map(|_| ()).map_err(|err| match err {
            sqlx::Error::Database(err) if matches!(err.code().as_deref(), Some("23505")) => {
    ...
    pub async fn _mark_cookie_as_linked(
        user: user::Id,
