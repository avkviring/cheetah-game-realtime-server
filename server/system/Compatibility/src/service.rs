...
mod tests {
	...
	async fn should_unsupported() {
		...
		let response = service
			.check_compatibility(Request::new(CheckCompatibilityRequest {
	...
	async fn should_planning_unsupported_soon() {
		let service = Service::new(
			vec![("0.0.1".to_owned(), Utc::now().add(Duration::hours(2)))]
				.into_iter()
				.collect(),
		);
	...
	async fn should_planning_unsupported_after_some_hours() {
		let service = Service::new(
			vec![("0.0.1".to_owned(), Utc::now().add(Duration::hours(6)))]
				.into_iter()
				.collect(),
		);
	...
	async fn should_supported() {
		let service = Service::new(
			vec![("0.0.1".to_owned(), Utc::now().add(Duration::days(2)))]
				.into_iter()
				.collect(),
		);
