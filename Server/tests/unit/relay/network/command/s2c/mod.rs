...
use cheetah_relay::network::command::s2c::{AffectedClients, S2CCommandUnion};
use cheetah_relay::network::command::s2c::delete_game_object::DeleteGameObjectS2CCommand;
use cheetah_relay::network::command::s2c::event::EventS2CCommand;
use cheetah_relay::network::command::s2c::S2CCommandCollector;
use cheetah_relay::network::command::s2c::update_float_counter::UpdateFloatCounterS2CCommand;
...
use cheetah_relay::room::room::{ClientId, Room};
...
fn test_affects_client() {
	...
	clients.clients.insert(0, Rc::new(client_stub_with_access_group(0, 0b1)));
	clients.clients.insert(1, Rc::new(client_stub_with_access_group(1, 0b100000)));
	clients.clients.insert(2, Rc::new(client_stub_with_access_group(2, 0b111)));
...
fn should_s2c_collect_on_object_create() {
	...
	room
		.create_root_game_object(10, &game_object_template_stub_with_group(0b100))
		.unwrap();
...
fn should_s2c_collect_on_client_connect() {
	...
	room.create_root_game_object(10, &game_object_template_stub_with_group(0b100));
	room.create_root_game_object(11, &game_object_template_stub_with_group(0b100));
	room.create_root_game_object(9, &game_object_template_stub_with_group(0b100));
	room.create_root_game_object(1, &game_object_template_stub_with_group(0b100));
...
fn should_s2c_collect_on_client_disconnect() {
	...
	room.create_client_game_object(
		&client_a,
		1,
		&game_object_template_stub_with_group(0b100),
	).unwrap();
	room.create_client_game_object(
		&client_a,
		2,
...
fn should_s2c_collect_on_update_long_counter() {
	...
	let id = room.create_client_game_object(&client, 1, &game_object_template_stub_with_group(0b100)).ok().unwrap();
