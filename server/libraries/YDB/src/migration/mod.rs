...
impl Migrator {
	...
	async fn get_applied_migrations(table_client: &TableClient) -> Result<HashMap<String, AppliedMigration>, MigrationError> {
		let result: Vec<(String, Vec<u8>)> = select!(table_client, query!("select * from migrations"),
			name=> String,
			checksum=> ydb::Bytes)
		.await
	...
	fn verify_migrations(&self, applied_migrations: &HashMap<String, AppliedMigration>) -> Result<(), MigrationError> {
		for migration in self.migrations.iter() {
			if let Some(applied_migration) = applied_migrations.get(&migration.name) {
				if applied_migration.checksum != migration.checksum {
					return Err(MigrationError::ChangeAppliedMigration(format!("name {}", migration.name)));
				}
			}
		}
		Ok(())
	...
	async fn mark_migration_as_applied(table_client: &TableClient, migration: &Migration) -> Result<(), MigrationError> {
		update!(
			table_client,
			query!(
	...
	async fn apply_migration(table_client: &TableClient, migration: &Migration) -> Result<(), MigrationError> {
		table_client
	...
	async fn create_migrated_table(table_client: &TableClient) -> Result<(), MigrationError> {
		table_client
			.retry_execute_scheme_query("create table migrations(name Utf8, checksum string,PRIMARY KEY(name));")
			.await
...
mod tests {
	...
	async fn should_migration() {
		...
		migrator.add_migration("002.sql".to_owned(), "alter table a ADD COLUMN some_flag Bool;".to_owned());
		migrator.add_migration("001.sql".to_owned(), "create table a(id int, PRIMARY KEY(id));".to_owned());
		migrator.migrate(&mut client).await.unwrap();
		client
			.table_client()
			.retry_transaction(|mut t| async move {
				t.query(Query::new("insert into a (id, some_flag) values (1, false)")).await?;
				Ok(())
			})
			.await
			.unwrap();
	...
	async fn should_not_migration_if_migrated() {
		let (_node, mut client) = get_or_create_ydb_instance("should_not_migration_if_migrated").await;
		let mut migrator = Migrator::default();
		migrator.add_migration("001.sql".to_owned(), "create table a(id int, PRIMARY KEY(id));".to_owned());
		migrator.migrate(&mut client).await.unwrap();
		migrator.migrate(&mut client).await.unwrap();
	...
	async fn should_not_migration_when_already_migrated_script_changed() {
		let (_node, mut client) = get_or_create_ydb_instance("should_not_migration_when_already_migrated_script_changed").await;
		{
			let mut migrator = Migrator::default();
			migrator.add_migration("001.sql".to_owned(), "create table a(id int, PRIMARY KEY(id));".to_owned());
			migrator.migrate(&mut client).await.unwrap();
		...
		let mut migrator = Migrator::default();
		migrator.add_migration("001.sql".to_owned(), "create table b(id int, PRIMARY KEY(id));".to_owned());
		assert!(migrator.migrate(&mut client).await.is_err());
	}

	#[tokio::test]
	async fn should_migration_from_directory() {
		let (_node, mut client) = get_or_create_ydb_instance("should_migration_from_directory").await;
