...
impl IntegrationTestHelper {
	...
	pub fn create_client(&self, member_id: RoomMemberId, user_key: &MemberPrivateKey) -> ClientId {
		let mut client: ClientId = 0;
		do_create_client(&self.socket_addr.to_string(), member_id, self.room_id, user_key, 0, &mut client);
		client
	}
	...
	pub fn create_user(&mut self) -> (RoomMemberId, MemberPrivateKey) {
		...
		let member_id = self.server.create_member(self.room_id, user_template).ok().unwrap();
...
pub fn setup<const N: usize>(builder: IntegrationTestServerBuilder) -> (IntegrationTestHelper, [u16; N]) {
	...
	for member in members.iter_mut() {
		...
		*member = helper.create_client(member_id, &user_key);
	}
	(helper, members)
}
