...
pub async fn setup(
	...
	let handler_user = tokio::spawn(async move {
		cheetah_auth_user::run_grpc_server(user_pool, format!("0.0.0.0:{}", internal_user_service_port).parse().unwrap()).await;
	});
	...
	(redis_container, container, handler_cerberus, handler_user, handler_google)
}
...
pub async fn should_register_and_login() {
	...
	let mut client = google::google_client::GoogleClient::connect(service_url).await.unwrap();
	...
	assert_eq!(
		token_parser.get_player_id(register_result.tokens.unwrap().session).unwrap(),
		token_parser.get_player_id(login_result.tokens.unwrap().session).unwrap(),
	);
}
...
pub async fn should_attach() {
	...
	let result: tonic::Response<google::RegisterOrLoginResponse> = google_client.register_or_login(request).await.unwrap();
	...
	assert_eq!(
		token_parser.get_player_id(token_from_cookie.session).unwrap(),
