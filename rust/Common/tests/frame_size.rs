...
use cheetah_common::protocol::frame::channel::ReliabilityGuaranteesChannel;
...
pub fn create_object_frame() {
	let mut frame = OutFrame::new(0, 100_500);
	...
	frame.add_command(CommandWithReliabilityGuarantees {
		reliability_guarantees: ReliabilityGuaranteesChannel::ReliableUnordered,
		commands: BothDirectionCommand::C2S(C2SCommand::SetStructure(SetStructureCommand {
			object_id: Default::default(),
			field_id: 30,
