...
// 	let id = room.create_client_game_object(client_id, 255, Option::None).ok().unwrap();
// 	let objects = room.objects.get(id).unwrap();
// 	assert_eq!(objects.groups.contains_group(0), true);
// 	assert_eq!(objects.groups.contains_group(5), true);
// 	assert_eq!(objects.groups.contains_group(7), false);
// }
//
// #[test]
...
// 	let (mut room, client_id, _) = setup_and_two_client();
// 	let result = room.create_client_game_object(client_id, 255, Option::Some(AccessGroups::from(&vec![3])));
// 	assert_eq!(result.is_err(), true)
// }
//
//
...
// 	let (mut room, _, _) = setup_and_two_client();
// 	let id = room.create_root_game_object(255, AccessGroups::from(&vec![])).ok().unwrap();
// 	assert_eq!(id, 255);
// }
