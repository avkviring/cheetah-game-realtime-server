...
pub struct RetransmitStatistics {
	...
	///
	asked_original_frames: LruCache<FrameId, bool>,
}
...
impl Default for RetransmitStatistics {
	fn default() -> Self {
		Self {
			...
			asked_original_frames: LruCache::new(RetransmitStatistics::FRAMES_STORAGE_LIMIT),
		}
	}
}
...
impl RetransmitStatistics {
	...
	///
	pub fn on_ask_received(&mut self, frame_id: FrameId, original_frame_id: FrameId, now: &Instant) {
		...
		if !self.asked_original_frames.contains(&original_frame_id) {
			self.asked_original_frames.put(original_frame_id, true);
			return;
		}
...
mod tests {
	...
	fn redundant_should_return_none_in_redundant_statistics() {
		...
		let now = Instant::now();
		statistics.on_ask_received(1, 100, &now);
	...
	fn redundant_should_return_average_2() {
		...
		let now = Instant::now();
		statistics.on_ask_received(1, 100, &now);
		statistics.on_ask_received(2, 101, &now);
		let now = now.add(RetransmitStatistics::MEASURE_DURATION);
		assert!(matches!(statistics.get_average_redundant_frames(&now), Option::Some(v) if v ==0));
	}
	...
	fn redundant_should_return_average_3() {
		...
		let now = Instant::now();
		statistics.on_ask_received(1, 100, &now);
		statistics.on_ask_received(2, 100, &now);
		statistics.on_ask_received(3, 100, &now);
	...
	fn redundant_should_return_average_different_cells() {
		...
		// 4 во вторую ячейку
		statistics.on_ask_received(10, 200, &now);
		statistics.on_ask_received(11, 200, &now);
		statistics.on_ask_received(12, 200, &now);
		statistics.on_ask_received(13, 200, &now);
		statistics.on_ask_received(14, 200, &now);
