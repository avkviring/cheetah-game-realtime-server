...
use crate::relay::room::room::{DeleteObjectError, Room};
...
fn delete_client_game_object() {
	let (mut room, clientA_id, clientB_id) = setup_and_two_client();
	let id = room.create_client_game_object(clientA_id, 255, &vec![]).ok().unwrap();
	let result = room.delete_game_object(clientA_id, id);
	assert_eq!(result.is_ok(), true);
	assert_eq!(room.objects.get(id).is_none(), true);
}

#[test]
fn delete_client_game_object_when_not_owner() {
	let (mut room, clientA_id, clientB_id) = setup_and_two_client();
	let id = room.create_client_game_object(clientA_id, 255, &vec![]).ok().unwrap();
	let result = room.delete_game_object(1010, id);
	assert_eq!(result.is_ok(), false);
	assert!(matches!(result.err().unwrap(), DeleteObjectError::ClientIsNotOwner));
	assert_eq!(room.objects.get(id).is_none(), false);
}

#[test]
fn delete_client_game_object_when_not_exists() {
	let (mut room, clientA_id, clientB_id) = setup_and_two_client();
	let result = room.delete_game_object(1010, 100500);
	assert_eq!(result.is_ok(), false);
	assert!(matches!(result.err().unwrap(), DeleteObjectError::GameObjectNotFound));
}
