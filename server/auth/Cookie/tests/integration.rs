...
pub async fn setup(
    ...
    let handler_user = tokio::spawn(async move {
        cheetah_auth_user::run_grpc_server(
            user_pool,
            format!("0.0.0.0:{}", user_port).parse().unwrap(),
        )
        .await;
    });
    ...
    let handler_cookie = tokio::spawn(async move {
        let cerberus_internal_service: Uri = format!("http://127.0.0.1:{}", internal_cerberus_port)
            .parse()
            .unwrap();
...
pub async fn should_registry_and_login_by_cookie() {
    ...
    assert_eq!(
        ...
        cookie::external::login_response::Status::Ok as i32
    );
