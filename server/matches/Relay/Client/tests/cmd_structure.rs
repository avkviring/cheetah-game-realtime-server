...
fn should_compare_and_set() {
	...
	ffi::command::structure::compare_and_set_structure(
		client2, &object_id, field_id_with_reset,
		&vec![0].into(), &vec![100].into(), true, &vec![42].into()
	);
	ffi::command::structure::compare_and_set_structure(
		client2, &object_id, field_id,
		&vec![0].into(), &vec![200].into(), false, &vec![0].into()
	);
	ffi::command::structure::compare_and_set_structure(
		client2, &object_id, field_id_with_reset,
		&vec![0].into(), &vec![200].into(), true, &vec![24].into()
	);
	helper.wait_udp();
...
extern "C" fn on_compare_and_set_listener(
	_: RoomMemberId, _object_id: &GameObjectIdFFI,
	field_id: FieldId, value: &BufferFFI
) {
