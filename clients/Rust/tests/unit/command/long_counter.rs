...
fn should_set_float_counter_from_ffi() {
	...
	assert!(matches!(&command,C2SCommandUnion::SetLongCounter(ref long_counter) if long_counter.global_object_id == 100));
...
fn should_increment_float_counter_from_ffi() {
	...
	assert!(matches!(&command,C2SCommandUnion::IncrementLongCounter(ref long_counter) if long_counter.global_object_id == 100));
