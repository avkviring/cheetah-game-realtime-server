...
use crate::unit::relay::room::objects::object::{game_object_template_stub_with_group, game_object_template_stub};
...
fn create_client_game_object_when_missing_client_group() {
	...
	let result = room
		.create_client_game_object(
			&client.clone(),
			255,
			&game_object_template_stub_with_group(0b1000),
		);
...
fn create_root_game_object() {
	...
	let id = room.create_root_game_object(255, &game_object_template_stub()).ok().unwrap();
	assert_eq!(id, 255);
}