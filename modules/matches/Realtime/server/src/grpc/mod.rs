                ...
                for room_id in &rooms {
                    if !present_rooms.contains(room_id) {
                        ...
                        tx.send(Ok(RoomLifecycleResponse {
                            room_id: *room_id,
                            r#type:
                            RoomLifecycleType::Created as i32,
                        })).await.unwrap();
                    }
                }
                ...
                for room_id in &present_rooms {
                    if !rooms.contains(&room_id) {
                        tx.send(Ok(RoomLifecycleResponse {
                            room_id: *room_id,
                            r#type:
                            RoomLifecycleType::Deleted as i32,
                        })).await.unwrap();
                    }
                }
                present_rooms.clear();
...
mod test {
    ...
    use crate::grpc::proto::internal::{DeleteMemberRequest, DeleteRoomRequest, EmptyRequest, GameObjectTemplatePermission, GetRoomInfoRequest, GroupsPermissionRule, MarkRoomAsReadyRequest, Permissions, PutForwardedCommandConfigRequest, UpdateRoomPermissionsRequest};
    use crate::grpc::proto::internal::realtime_server::Realtime;
    use crate::grpc::proto::internal::room_lifecycle_response::RoomLifecycleType;
