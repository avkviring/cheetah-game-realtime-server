...
pub fn setup_client(room: &mut Room, client_hash: &str, group: GroupType) -> Rc<Client> {
	room.add_client_to_waiting_list(&client_hash.to_hash_value(), AccessGroups::from(group));
	let second_client = room.client_connect(&client_hash.to_hash_value()).unwrap();
	second_client
}
