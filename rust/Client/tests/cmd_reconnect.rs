...
fn should_reconnect() {
	...
	let reconnected_client = helper.create_client(member_id, &private_key, 5);
	ffi::command::room::attach_to_room(reconnected_client);
	helper.wait_udp();
	let commands = helper.receive(reconnected_client);
